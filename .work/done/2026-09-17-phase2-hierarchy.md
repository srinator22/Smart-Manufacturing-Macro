...
- Route (a), caught pre-merge by the reviewer gate: F1 quadratic aggregation, F2 transient tri-state with duplicate notification, R1 the same class surviving for repeated sub-assemblies, R2 a guard that could not fail, plus F3/F4 weak gate checks. The gate worked; each was repaired with a failing-first regression before the fix. Nothing recorded per step 2, with one exception noted below.
- Route (d), self-noticed and self-corrected: D1 dead row view model, D2 hardcoded scope combo box, D3 mutation scoped to Core with `--break-at 0`, D4 the window crashing on open, D5 the duplicate-occurrence checkbox that silently exported anyway. Nothing recorded per step 2, with one exception noted below.
- Route (b) escaped past merge: none. Route (c) reported by the human: none.

Two exceptions proposed to docs/lessons/PENDING.md rather than dropped, because in both cases the route classification hides the real signal:

- D4 was route (d), but it survived 48 unit tests, a full green `scripts/check.sh`, an independent review, and three GitHub review rounds. Nothing in the gate had ever opened the window. "The system worked" is false for that defect; what worked was rendering it by hand.
- F1 was route (a), but it was the third recurrence of one class on one file, each instance caught by a reviewer and each repaired without a check that could see it. A class that recurs three times through the review gate is evidence of a missing mechanized check, not of a working system.

Regression tests verified present and observed failing before their fix: `SmartExportWindowShowsTheHierarchyTemplateWithoutBindingFailures` (D4, and proven to gate by commenting out the template), `ScopeOptionsExposeEveryScopeModeWithADistinctDisplayName` (D2), `DeselectingOneOccurrenceDeselectsEveryOccurrenceOfTheSameDocument` and `SelectingOneOccurrenceSelectsEveryOccurrenceOfTheSameDocument` and `DeselectingAnOccurrenceRecomputesAncestorsInEveryBranchThatSharesTheDocument` (D5), `TogglingOneOccurrenceOfAWidelyRepeatedDocumentStaysLinearInTreeSize` (F1), `TogglingAnOccurrenceNotifiesSharedAncestorsOnceWithTheFinalValue` (F2), `TogglingARepeatedSubassemblyNotifiesEveryNodeAtMostOnceWithTheFinalValue` (R1).

docs/lessons/INDEX.md has no approved lessons, so there was no `used:` counter to increment (step 6).

Delivery evidence: local `scripts/check.sh` exit 0 with `check: OK`; PR #9 CI terminal-success for the exact pushed SHA 1d0d7475fb2beca1568fa03b08276ad4faedf362; merged as 9d0d19051dea945b226c85fea7c30921f3ebbb6c; main CI terminal-success for that exact merge SHA.

Not claimed: live Inventor 2027 acceptance. The add-in has still never been exercised against real Inventor, and the three P0 live-acceptance rows in BACKLOG.md remain open.
