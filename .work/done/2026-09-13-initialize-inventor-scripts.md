...
- [x] The initialized commit is tagged v0.1.0 and CI/branch protection are verified for the exact pushed SHA -> scripts/ci-watch.sh and GitHub API
...
- 2026-09-13: PR #1 merged to protected main at `c901b9a88528f74d967552e50226a25056b664df`; main and annotated `v0.1.0` CI both reached terminal success for that exact SHA.
- 2026-09-13: Published GitHub release `v0.1.0 - Inventor 2027 workspace foundation` without claiming an installable add-in.
...
- Pre-merge review caught exporter-specific mutation routing, acceptance of empty mutation runs, and missing cross-project reference enforcement. The owning checks were added and the reviewer passed the repair.
- Pre-merge CI caught changelog output that depended on local-only template tags. The freshness gate now uses exact SemVer tags and the declared workspace version; clean-clone, tagged-clone, local, main, and release-tag runs agree.
- GitHub runner acquisition and merge API failures were external transient failures. The protected fast-forward used the already green exact SHA, and GitHub recorded PR #1 as merged.
- Escaped after merge: post-release PR CI found that forcing an already-existing release tag duplicated an empty heading for GitHub's synthetic merge commit. `scripts/test-changelog.sh` now reproduces that merge shape and compares byte-for-byte output. The generator removes only an empty Unreleased header, forces the declared tag only before release, and uses the real tag boundary afterward. Pull-request CI explicitly checks the head SHA; strict protection still requires the branch to be current with `main`.
- One defect escaped past merge and is now guarded by the failing-then-fixed changelog regression test. No human-reported defect occurred, and no prose lesson is warranted because the executable guard expresses the cause.
- No approved lesson entry was consulted, so no lesson usage counter changed.
