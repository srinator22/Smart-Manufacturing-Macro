...
try {
    ...
    $manifestPath = Join-Path $resolvedTestRoot "Autodesk.WmpToolsManager.Inventor.addin"
    $assemblyPath = Join-Path $resolvedTestRoot "WmpToolsManager\WmpToolsManager.AddIn.dll"
    ...
    if ($manifest.Addin.ClassId -ne "{39625833-F960-4BDE-9EB1-F1E7F8F8013B}") {
    ...
    if (Test-Path -LiteralPath (Join-Path $resolvedTestRoot "WmpToolsManager")) {
