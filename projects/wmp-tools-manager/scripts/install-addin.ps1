...
$sourceDirectory = Join-Path $projectRoot "src\WmpToolsManager.AddIn\bin\$Configuration\net10.0-windows"
$sourceAssembly = Join-Path $sourceDirectory "WmpToolsManager.AddIn.dll"
$manifestTemplate = Join-Path $projectRoot "packaging\Autodesk.WmpToolsManager.Inventor.addin.template"
...
$installDirectory = Join-Path $resolvedAddinsRoot "WmpToolsManager"
$installedAssembly = Join-Path $installDirectory "WmpToolsManager.AddIn.dll"
$installedManifest = Join-Path $resolvedAddinsRoot "Autodesk.WmpToolsManager.Inventor.addin"

if ($PSCmdlet.ShouldProcess($installDirectory, "Install WMP Tools Manager for the current user")) {
