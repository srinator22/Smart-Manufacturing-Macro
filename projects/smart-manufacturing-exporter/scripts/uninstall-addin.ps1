...
$installDirectory = [System.IO.Path]::GetFullPath((Join-Path $resolvedAddinsRoot "SmartManufacturingExporter"))
$installedManifest = [System.IO.Path]::GetFullPath(
    (Join-Path $resolvedAddinsRoot "Autodesk.SmartManufacturingExporter.Inventor.addin"))
...
if ((Test-Path -LiteralPath $installedManifest) -and
    $PSCmdlet.ShouldProcess($installedManifest, "Remove Smart Manufacturing Exporter manifest")) {
    Remove-Item -LiteralPath $installedManifest -Force
}
if ((Test-Path -LiteralPath $installDirectory) -and
    $PSCmdlet.ShouldProcess($installDirectory, "Remove Smart Manufacturing Exporter binaries")) {
    Remove-Item -LiteralPath $installDirectory -Recurse -Force
}

Write-Output "Smart Manufacturing Exporter is removed from '$resolvedAddinsRoot'."
