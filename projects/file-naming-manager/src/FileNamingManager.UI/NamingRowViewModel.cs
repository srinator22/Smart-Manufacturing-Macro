...
// Outputs: Read-only display strings; ProposedFileName/Action/Reasons raise PropertyChanged so the
//   DataGrid updates in place without reopening the window.
...
public sealed class NamingRowViewModel : INotifyPropertyChanged
{
    ...
    {
        get => action;
        private set => SetField(ref action, value, nameof(Action));
    }
