...
// Outputs: Report rows/findings for display, a live RenamePlan re-computed as the project number or
//   options change, and an Apply() that executes that plan and reports the outcome.
...
//   claiming an action the plan will not perform is a defect (D6), not a preview nuance. A WPF window
...
public sealed class FileNamingViewModel : INotifyPropertyChanged
{
    ...
    public void Apply()
    {
        ...
        plan = null;
        RefreshRows(new RenameOptions(RenameUnnumbered, NormalizeMalformed, SetPartNumberProperty));
    ...
    private void ValidateAndPlan()
    {
        ...
        RenameOptions options = new(RenameUnnumbered, NormalizeMalformed, SetPartNumberProperty);
