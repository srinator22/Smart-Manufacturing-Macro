...
$installDirectory = [System.IO.Path]::GetFullPath((Join-Path $resolvedAddinsRoot "FileNamingManager"))
$installedManifest = [System.IO.Path]::GetFullPath(
    (Join-Path $resolvedAddinsRoot "Autodesk.FileNamingManager.Inventor.addin"))
...
if ((Test-Path -LiteralPath $installedManifest) -and
    $PSCmdlet.ShouldProcess($installedManifest, "Remove File Naming Manager manifest")) {
    Remove-Item -LiteralPath $installedManifest -Force
}
if ((Test-Path -LiteralPath $installDirectory) -and
    $PSCmdlet.ShouldProcess($installDirectory, "Remove File Naming Manager binaries")) {
    Remove-Item -LiteralPath $installDirectory -Recurse -Force
}

Write-Output "File Naming Manager is removed from '$resolvedAddinsRoot'."